...
function MapPosePage({ mapName, onBack }) {
  ...
  useEffect(() => {
    fetch(`http://172.27.232.2:8000/navigation/list/pose/${encodeURIComponent(mapName)}`)
  ...
  useEffect(() => {
    const fetchRobotLocation = async () => {
        try {
            const response = await fetch('http://172.27.232.2:8000/navigation/goal/feedback');
            if (!response.ok) {
  ...
  const handleAddPose = () => {
    fetch('http://172.27.232.2:8000/navigation/new/pose', {
      method: 'POST',
      headers: {
  ...
  const handleNavigationGoalStart = async () => {
    const response_navigation = await fetch('http://172.27.232.2:8000/navigation/goal/start', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const handleNavigationGoalStop = async () => {
    const response_cancel = await fetch('http://172.27.232.2:8000/navigation/goal/cancel', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const handleGoalPoseDetails = async (pose) => {
    const response = await fetch('http://172.27.232.2:8000/navigation/pose', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const handleSetInitialPose = () => {
    fetch('http://172.27.232.2:8000/navigation/initial_pose', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
